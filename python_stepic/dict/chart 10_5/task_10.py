#  переменной s хранится строка пар число:слово. Дополните приведенный код, используя генератор,
...
result = {int(key): value for key, value in [char.split(':')for char in s.split()]}
